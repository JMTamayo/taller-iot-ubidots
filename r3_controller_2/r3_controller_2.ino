/*
  ...
  Links de Interés:
    - Protocolos Serial2es:
      https://docs.arduino.cc/micropython/micropython-course/course/Serial2/

  Placa de Desarrollo:
    - Nombre: ARDUINO MEGA
    - Documentación: https://docs.arduino.cc/hardware/mega-2560/
*/

// Etiquetado de Pines I/O:
#define SOIL_MOISTURE_SENSOR_PIN                                               \
  A0 // Pin para lectura analógica de sensor de humedad
#define WATER_FLOW_ACTUATOR_PIN                                                \
  2 // Pin de salida digital para control de sistema de bombeo

// Definición de Constantes:
#define BAUD_RATE                                                              \
  9600 // Velocidad de comunicación Serial2 con dispositivo principal

#define FARM_ID "f2" // Identificador único de la finca
...
#define SOIL_MOISTURE_SENSOR_MIN_VALUE                                         \
  420 // Valor mínimo del sensor de humedad, medido en agua
#define SOIL_MOISTURE_SENSOR_MAX_VALUE                                         \
  1023 // Valor máximo del sensor de humiedad, medido en aire
...
bool unwrapRequest(String request) {
  ...
  if (separatorIndex == -1) {
    Serial2.print("error:");
    Serial2.println("The request could not be interpreted, it must be in the "
                    "format {command}" +
  ...
  if (Serial2.available() > 0) {
    ...
    if (!error) {
      if (requestCommand.equals(WATER_FLOW_ACTUATOR_COMMAND) &&
        ...
        Serial2.println(requestCommand + String("-") + FARM_ID +
